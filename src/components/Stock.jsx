...
*/
  ...
  const numericalChange = Number((currentPrice - prevClosingPrice).toFixed(2));

  const rateChange = Number(((numericalChange / prevClosingPrice) * 100).toFixed(2));

  const colorClass = currentPrice > prevClosingPrice ? "green" : "red";

  const arrow = currentPrice > prevClosingPrice ? "⬆" : currentPrice < prevClosingPrice ? "⬇" : "▬";
